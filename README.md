...
Crossref身份核验同时处理独立subtitle字段和题名内的JATS行内标记，完整题名与DOI必须一致，不使用前缀或模糊匹配放行。匹配规则修订时对旧版误差候选重新核验。PMID和规范化DOI用于稳定去重；相似题名不能强制合并，标识冲突独立保存在 `conflicts`。预印本和正式版保留各自条目，仅当Crossref返回明确关系时关联。更正、撤稿、关注声明醒目标记。
